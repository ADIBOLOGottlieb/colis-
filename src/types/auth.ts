...
/**
 * Check if user can create colis packages
 */
export function canCreateColis(user: UserWithRole): boolean {
  try {
    const effectiveMode = getEffectiveMode(user);
    return hasPermission(effectiveMode, Permission.CREATE_COLIS);
  } catch {
    return false;
  }
}
...
/**
 * Check if user can create trajet trips
 */
export function canCreateTrajet(user: UserWithRole): boolean {
  try {
    const effectiveMode = getEffectiveMode(user);
    return hasPermission(effectiveMode, Permission.CREATE_TRAJET);
  } catch {
    return false;
  }
}
...
/**
 * Check if user can edit colis
 */
export function canEditColis(user: UserWithRole, colisOwnerId: string): boolean {
  try {
    if (user.id !== colisOwnerId) return false;
    const effectiveMode = getEffectiveMode(user);
    return hasPermission(effectiveMode, Permission.EDIT_OWN_COLIS);
  } catch {
    return false;
  }
}
...
/**
 * Check if user can edit trajet
 */
export function canEditTrajet(user: UserWithRole, trajetOwnerId: string): boolean {
  try {
    if (user.id !== trajetOwnerId) return false;
    const effectiveMode = getEffectiveMode(user);
    return hasPermission(effectiveMode, Permission.EDIT_OWN_TRAJET);
  } catch {
    return false;
  }
}
...
/**
 * Check if user can delete colis
 */
export function canDeleteColis(user: UserWithRole, colisOwnerId: string): boolean {
  try {
    if (user.id !== colisOwnerId) return false;
    const effectiveMode = getEffectiveMode(user);
    return hasPermission(effectiveMode, Permission.DELETE_OWN_COLIS);
  } catch {
    return false;
  }
}
...
/**
 * Check if user can delete trajet
 */
export function canDeleteTrajet(user: UserWithRole, trajetOwnerId: string): boolean {
  try {
    if (user.id !== trajetOwnerId) return false;
    const effectiveMode = getEffectiveMode(user);
    return hasPermission(effectiveMode, Permission.DELETE_OWN_TRAJET);
  } catch {
    return false;
  }
}
...
/**
 * Check if user can view colis (all authenticated users can view)
 */
export function canViewColis(user: UserWithRole): boolean {
  try {
    const effectiveMode = getEffectiveMode(user);
    return hasPermission(effectiveMode, Permission.VIEW_COLIS);
  } catch {
    return false;
  }
}
...
/**
 * Check if user can view trajets (all authenticated users can view)
 */
export function canViewTrajets(user: UserWithRole): boolean {
  try {
    const effectiveMode = getEffectiveMode(user);
    return hasPermission(effectiveMode, Permission.VIEW_TRAJETS);
  } catch {
    return false;
  }
}
...
/**
 * Check if user can initiate contact with voyageurs
 */
export function canContactVoyageur(user: UserWithRole): boolean {
  try {
    const effectiveMode = getEffectiveMode(user);
    return hasPermission(effectiveMode, Permission.CONTACT_VOYAGEUR);
  } catch {
    return false;
  }
}
...
/**
 * Check if user can initiate contact with expéditeurs
 */
export function canContactExpediteur(user: UserWithRole): boolean {
  try {
    const effectiveMode = getEffectiveMode(user);
    return hasPermission(effectiveMode, Permission.CONTACT_EXPEDITEUR);
  } catch {
    return false;
  }
}
...
/**
 * Check if user can switch between EXPEDITEUR and VOYAGEUR modes
 * Only LES_DEUX users can switch modes
 */
export function canSwitchMode(user: UserWithRole): boolean {
  return user.role === UserRole.LES_DEUX;
}

/**
 * Get user's effective role for display/sorting purposes
 */
export function getEffectiveRole(user: UserWithRole): ActiveMode {
  try {
    return getEffectiveMode(user);
  } catch {
    return ActiveMode.EXPEDITEUR; // Default fallback
  }
}
...
export function buildSessionUser(user: UserWithRole): SessionUser {
  ...
  return {
    ...user,
    effectiveRole,
    canCreateColis: hasPermission(effectiveRole, Permission.CREATE_COLIS),
    canCreateTrajet: hasPermission(effectiveRole, Permission.CREATE_TRAJET),
    canSwitchMode: user.role === UserRole.LES_DEUX
  };
}
