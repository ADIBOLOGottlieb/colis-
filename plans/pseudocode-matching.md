...
export async function findMatchesForColis(
  ...
  const matches: MatchResult[] = candidates.map(trajet =>
    calculateMatch({
      colis: {
        ...colis,
...
export async function findMatchesForTrajet(
  ...
  // Calculate scores
  const matches: MatchResult[] = candidates.map(colis =>
    calculateMatch({
      colis: {
        ...colis,
...
function validateMatchInput(input: MatchInput): void {
  ...
  if (typeof colis.poids !== 'number' || colis.poids <= 0) {
    throw new Error('Invalid colis: poids must be positive number');
  }
  ...
  if (!trajet.dateVoyage || !isValid(trajet.dateVoyage)) {
    throw new Error('Invalid trajet: dateVoyage must be valid date');
  }
  if (typeof trajet.kilosDisponibles !== 'number' || trajet.kilosDisponibles < 0) {
    throw new Error('Invalid trajet: kilosDisponibles must be non-negative');
  }
}

/**
 * Convert MatchResult to summary format for list views
 */
export function toMatchSummary(result: MatchResult): MatchSummary {
  return {
    colisId: result.colisId,
    trajetId: result.trajetId,
    score: result.score,
    isRecommended: result.isRecommended,
    departureMatch: result.breakdown.primary.departure.matched,
    arrivalMatch: result.breakdown.primary.arrival.matched,
    dateMatch: result.breakdown.primary.date.matched,
    weightCompatible: result.breakdown.secondary.weightCompatibility.canAccommodate
  };
}
```
